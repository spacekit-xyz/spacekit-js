...
import { substituteEnvInChainConfig, loadProofBridgeConfig, createAdaptersFromConfig, } from "./proof_bridge.js";
...
test("loadProofBridgeConfig url throws on fetch failure", async () => {
    ...
    try {
        await assert.rejects(async () => loadProofBridgeConfig({
            source: "url",
...
test("createAdaptersFromConfig uses custom registry", async () => {
    ...
    const registry = {
        create: async (config) => config.chainId === "custom" ? fakeAdapter : null,
    };
...
test("createAdaptersFromConfig with registry returning null yields empty list", async () => {
    const adapters = await createAdaptersFromConfig([{ enabled: true, chainId: "ethereum" }], { adapterRegistry: { create: async () => null } });
