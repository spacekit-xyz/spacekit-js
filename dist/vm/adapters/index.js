 ...
 */
import { setDefaultAdapterRegistry, } from "../proof_bridge.js";
...
async function createAdapter(config) {
    ...
    if (id.startsWith("ethereum"))
        return createEthereumAdapter(config);
    if (id.startsWith("bitcoin"))
        return createBitcoinAdapter(config);
    if (id.startsWith("solana"))
        return createSolanaAdapter(config);
    return null;
}
const registry = {
    async create(config) {
