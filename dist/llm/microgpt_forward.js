...
const LAYER0_ATTN_WQ = [
    [0.1, 0.02, 0.01, -0.01],
    [0.0, 0.08, -0.02, 0.03],
    [-0.01, 0.03, 0.09, 0.01],
    [0.02, -0.01, 0.02, 0.07],
];
const LAYER0_ATTN_WK = [
    [0.09, 0.01, 0.0, -0.02],
    [0.01, 0.07, -0.01, 0.02],
    [-0.02, 0.02, 0.08, 0.0],
    [0.03, -0.01, 0.01, 0.06],
];
const LAYER0_ATTN_WV = [
    [0.08, 0.02, -0.01, 0.01],
    [0.0, 0.06, 0.0, 0.02],
    [-0.01, 0.01, 0.07, 0.0],
    [0.02, 0.0, 0.01, 0.05],
];
const LAYER0_ATTN_WO = [
    [0.09, 0.01, 0.0, -0.01],
    [0.01, 0.07, -0.01, 0.02],
    [-0.01, 0.02, 0.08, 0.0],
    [0.02, -0.01, 0.01, 0.06],
];
const LAYER0_MLP_FC1 = [
    [0.1, 0.02, 0.01, -0.01],
    [0.0, 0.08, -0.02, 0.03],
    [-0.01, 0.03, 0.09, 0.01],
    [0.02, -0.01, 0.02, 0.07],
...
function linear(out, x, w, inDim, outDim) {
...
export function microgpt_forward(token_id, pos_id) {
    ...
    for (let j = 0; j < HEAD_DIM; j++)
        xAttn[j] = attnWeight * v[j];
    const xProj = new Array(N_EMBD);
    linear(xProj, xAttn, LAYER0_ATTN_WO, N_EMBD, N_EMBD);
    for (let i = 0; i < N_EMBD; i++)
        x[i] = xProj[i] + xResidual[i];
    const xResidual2 = x.slice();
    rmsnorm(x);
    const h = new Array(4 * N_EMBD);
