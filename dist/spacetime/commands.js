export function parseSpaceTimeCommand(input) {
    ...
    if (header === "spacetime:new-thread") {
        ...
        const blankIndex = lines.findIndex((line, idx) => idx > 1 && line.trim() === "");
        const bodyLines = blankIndex === -1 ? lines.slice(2) : lines.slice(blankIndex + 1);
    ...
    if (header === "spacetime:reply") {
        ...
        const parentPostId = parentRaw === "none"
            ? null
            : Number.isFinite(Number(parentRaw))
                ? Number(parentRaw)
                : null;
        const blankIndex = lines.findIndex((line, idx) => idx > 2 && line.trim() === "");
        const bodyLines = blankIndex === -1 ? lines.slice(3) : lines.slice(blankIndex + 1);
        const text = bodyLines.join("\n").trim();
        if (!text) {
            return null;
        }
