...
import { loadProofBridgeConfig, createAdaptersFromConfig, } from "../vm/proof_bridge.js";
...
async function pollStorage(adapters, baseUrl, did, collection, seenIds) {
    ...
    for (const doc of docs) {
        ...
        const payload = signed
            ? { kind: "signed_bundle", signed: bundle }
            : { kind: "bundle", bundle };
...
async function pollRpc(adapters, rpcUrl, lastHeightRef) {
    ...
    if (height <= lastHeightRef.current || !stateRoot)
        return;
    lastHeightRef.current = height;
    const payload = { kind: "state_root", blockHeight: height, stateRoot, quantumStateRoot };
    for (const adapter of adapters) {
        if (!adapter.isReady())
            continue;
        try {
...
async function main() {
    ...
    if (source === "storage") {
        ...
        if (!rpcUrl)
            throw new Error("For source=rpc set SPACEKIT_RPC_URL");
