...
export async function installPolyfills() {
    ...
    if (typeof globalThis.indexedDB === "undefined") {
        const useWal = typeof process !== "undefined" &&
            process.env?.SPACEKIT_IDB_BACKEND === "wal";
        if (useWal) {
            try {
                const { createIDBFactory } = await import("./idb-node/index.js");
                const basePath = typeof process !== "undefined" && process.env?.SPACEKIT_IDB_PATH
                    ? process.env.SPACEKIT_IDB_PATH
                    : ".spacekit-idb";
                globalThis.indexedDB = createIDBFactory(basePath);
            }
            catch (e) {
                throw new Error(`[spacekit] WAL IndexedDB backend failed: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
        else {
            try {
                ...
                throw new Error(`[spacekit] IndexedDB polyfill not found.\n` +
                    `Install it with:\n` +
 ...
 */
export function resetPolyfillState() {
