...
PRIMITIVES="$ROOT/spacekit-primitives/wasm-kyber"
...
# Note: Using getrandom 0.2 with "js" feature, no RUSTFLAGS needed
cargo build --release --target wasm32-unknown-unknown

# Generate JS bindings directly into dist/wasm
mkdir -p "$OUT_DIST"
wasm-bindgen --target web --out-dir "$OUT_DIST" --out-name kyber_wasm \
  target/wasm32-unknown-unknown/release/spacekit_kyber_wasm.wasm

# Optionally copy to a website public folder if WEBSITE_WASM_OUT is set
if [ -n "$WEBSITE_OUT" ]; then
  mkdir -p "$WEBSITE_OUT"
  cp "$OUT_DIST/kyber_wasm.js" "$WEBSITE_OUT/"
  cp "$OUT_DIST/kyber_wasm_bg.wasm" "$WEBSITE_OUT/"
fi
